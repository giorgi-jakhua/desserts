import { CarbonNeutral } from "../Icons";

const Cart = ({ quantities, products, removeFromCart }) => {
  const cartItems = products.filter((product) => quantities[product.name]);
  // ამ ფუნქციას გამოაქვს ახალი მასივი რომელშიც შედის cart-ში დამატებული პროდუქტებისთვის ობიექტები, რომლებშიც შედის ის გასაღებები და მნიშვნელობები რომლებიც შესაბამის პროდუქტს აქვს data.json-ში
  return (
    <div className="cart">
      <h1>Your Cart ({Object.keys(quantities).length})</h1>
      {/* რამდენი გასაღებიც არის quantities ობიექტში, ის რიცხვი გამოგვაქვს */}
      {cartItems.length > 0 ? (
        <div className="cart-items">
          {/* map ფუნქციით cartItems მასივში შემავალი თითოეული ელემენტისთვის გამოგვაქვს cart-item-box */}
          {cartItems.map((item) => (
            <div key={item.name} className="cart-item-box">
              <div>
                <h3>{item.name}</h3>
                <div className="cart-item-price-and-quantity">
                  <p>{quantities[item.name]}x</p>
                  <p1>@ ${item.price.toFixed(2)}</p1>
                  <p2>${(item.price * quantities[item.name]).toFixed(2)}</p2>
                </div>
              </div>
              <button className="remove-btn" onClick={() => removeFromCart(item.name)}><i class="fi fi-br-cross"></i></button>
            </div>
          ))}
          <div className="order-total">
            <p>Order total</p>
            <h1>${cartItems.reduce((total, item) => total + (item.price * quantities[item.name]), 0)}</h1>
            {/* reduce ფუნქცია მასივში შემავალი თითოეული ელემენტისთვის ასრულებს ფუქციას და ბოლოს გამოაქვს ერთი მნიშვნელობა. ამ შემთხვევაში total ცვლადს(რომლის მნიშვნელობაც თავდაპირველად 0 არის) ემატება მასივში შემავალი ელემენტის ფასი გამრავლებული მის რაოდენობაზე და ბოლოს გამოაქვს რისი ტოლიც იქნება total ცვლადი.*/}
          </div>
          <div className="carbon-neutral"><CarbonNeutral/> This is a <p className="bold-text">carbon-neutral</p> delivery</div>
      ...
      {/* თუ cart-ში რომელიმე დესერტი გვაქვს დამატებული(ანუ თუ cartItems მასივის სიგრძე მეტია 0-ზე) გამოაქვს cart-items ყუთი, ხოლო თუ არ გვაქვს empty-cart ყუთი. */}
    </div>
  );
};
